...
############################################################################################################################################
    ...
    elseif ($AppName -eq "CreativeCloud"){
        try {
            # Get latest version from GitHub API
    ...
    # Find the InstallerUrls for x64 and arm64
    $patternX64 = 'InstallerUrl:\s*(\S*/download\?sapCode=KCCC&wamFeature=nuj-live)'
    $patternARM64 = 'InstallerUrl:\s*(\S*/download\?sapCode=KCCC&wamFeature=nuj-live)'
    ...
    if ($arch -like "*ARM*") {
        Invoke-LogMessage "Downloaded arm64 installer to $local:filearm64"
        $renamedFile = Join-Path -Path $local:folderPath -ChildPath "CreativeCloud_Installer_arm64.exe"
        Rename-Item -Path $local:filearm64 -NewName $renamedFile
        Invoke-LogMessage "Renamed installer to: $renamedFile"
        Start-Process -FilePath $renamedFile -ArgumentList "--mode=stub" -Wait -ErrorAction Stop
        Remove-Item -Path $renamedFile -Force # Remove the renamed MSI after installation
        Invoke-LogMessage "Removed installer: $renamedFile"
    } else {
        Invoke-LogMessage "Downloaded x64 installer to $local:filex64"
        $renamedFile = Join-Path -Path $local:folderPath -ChildPath "CreativeCloud_Installer_x64.exe"
        Rename-Item -Path $local:filex64 -NewName $renamedFile
        Invoke-LogMessage "Renamed installer to: $renamedFile"
        Start-Process -FilePath $renamedFile -ArgumentList "--mode=stub" -Wait -ErrorAction Stop
        Remove-Item -Path $renamedFile -Force # Remove the renamed MSI after installation
    ...
    Invoke-LogMessage "Successfully installed Adobe Creative Cloud."
} catch {
    Invoke-LogMessage "Error installing Adobe Creative Cloud: $($_.Exception.Message)"
}
    }
}
############################################################################################################################################
