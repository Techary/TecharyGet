...
############################################################################################################################################
    ...
    elseif ($AppName -eq "ArcBrowser"){
        try {
            # Get latest version from GitHub API
            $apiUrl = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/t/TheBrowserCompany/Arc"
            $headers = @{
                "User-Agent" = "PowerShell"
                "Accept" = "application/vnd.github.v3+json"
            }
            ...
            # Build YAML URL from Github to gather Installation URL
            $yamlUrl = "https://raw.githubusercontent.com/microsoft/winget-pkgs/master/manifests/t/TheBrowserCompany/Arc/${latestVersion}/TheBrowserCompany.Arc.installer.yaml"
            Invoke-LogMessage "Downloading YAML from: $yamlUrl"
            ...
            # Find the InstallerUrls for x64 and arm64
            $patternX64 = 'InstallerUrl:\s*(\S*/Arc\.x64\.msix)'
            $patternARM64 = 'InstallerUrl:\s*(\S*/Arc\.arm64\.msix)
        '
            ...
            if ($arch -like "*ARM*") {
                Invoke-LogMessage "Downloaded arm64 installer to $local:filearm64"
                $renamedFile = Join-Path -Path $local:folderPath -ChildPath "ArcBrowser_Installer_arm64.msix"
                Rename-Item -Path $local:filearm64 -NewName $renamedFile
                Invoke-LogMessage "Renamed installer to: $renamedFile"
                Add-AppxPackage -Path $renamedFile -ForceUpdateFromAnyVersion -ErrorAction Stop
                Remove-Item -Path $renamedFile -Force # Remove the renamed MSI after installation
                Invoke-LogMessage "Removed installer: $renamedFile"
            } else {
                Invoke-LogMessage "Downloaded x64 installer to $local:filex64"
                $renamedFile = Join-Path -Path $local:folderPath -ChildPath "ArcBrowser_Installer_x64.msix"
                Rename-Item -Path $local:filex64 -NewName $renamedFile
                Invoke-LogMessage "Renamed installer to: $renamedFile"
                Add-AppxPackage -Path $renamedFile -ForceUpdateFromAnyVersion -ErrorAction Stop
                Remove-Item -Path $renamedFile -Force # Remove the renamed MSI after installation
            ...
            Invoke-LogMessage "Successfully installed Arc Browser."
        } catch {
            Invoke-LogMessage "Error installing Arc Browser: $($_.Exception.Message)"
        }
    }
############################################################################################################################################
############################################################################################################################################
